...
# schema.py is bundled alongside this file in the container
...
IDENTIFIER_COL = schema.identifier_column()
...
def get_prediction_bucket(probability: float) -> str:
    """Map a predicted positive-class probability to a human-readable bucket.

    Buckets are symmetric around 0.5 and work for any binary classifier:
        [0.0, 0.2) -> very_low
        [0.2, 0.4) -> low
        [0.4, 0.6) -> medium
        [0.6, 0.8) -> high
...
def predict_fn(input_data: pd.DataFrame, model_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results = {
        ...
        "probabilities": {
            "negative": (1 - probabilities).tolist(),
            "positive": probabilities.tolist(),
        },
    }
    ...
    if ENABLE_ATHENA_LOGGING and SQS_QUEUE_URL:
        try:
            ...
            for idx in range(len(predictions)):
                positive_prob = float(results["probabilities"]["positive"][idx])
                confidence_score = max(positive_prob, 1 - positive_prob)
                ...
                log_entry = {
                    ...
                    'probability_negative': float(1 - positive_prob),
                    ...
                    IDENTIFIER_COL: str(original_data.iloc[idx].get(IDENTIFIER_COL, '')) or None,
                    ...
                    'prediction_bucket': get_prediction_bucket(positive_prob),
